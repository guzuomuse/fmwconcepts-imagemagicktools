#!/bin/bash
#
# Developed by Fred Weinhaus 8/27/2007 .......... revised 7/16/2009
#
...
gaussian()
	...
	loArr=(`awk -v mean="$mean" -v fact="$fact" -v expo="$expo" '
		BEGIN { for (i=0;i<=mean;i++) print expo^(-((i-mean)^2)*fact); }'`)
#	echo ${loArr[*]}
#	echo ${#loArr[*]}
	...
	hiArr=(`awk -v mean="$mean" -v fact="$fact" -v expo="$expo" '
		BEGIN { for (i=mean+1;i<256;i++) print expo^(-((i-mean)^2)*fact); }'`)
#	echo ${hiArr[*]}
#	echo ${#hiArr[*]}
...
tmp0="$dir/redist_0_$$.png"
tmp1="$dir/redist_1_$$.png"
tmp2="$dir/redist_2_$$.png"
tmpP="$dir/redist_proc_$$.png"
tmp3="$dir/redist_3_$$.png"
...
if [ $display_graph = "yes" ]
	...
	if [ "$shape" != "Uniform" -a "$shape" != "uniform" ]
		...
		while [ $i -lt 256 ]
			do
			x=`expr $i / 2`
			y=`echo "scale=0; 100 - (100 * ${functionArr[$i]} / $functionmaxval)" | bc`
			pointArr[$x]="$x,$y"
			i=`expr $i + 2`
		done
