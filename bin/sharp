#!/bin/bash
#
# Developed by Fred Weinhaus 10/05/2007 .......... revised 5/8/2012
#
...
trap "rm -f $tmpA $tmpB $tmpM $tmpS; exit 0" 0
...
# get mean
convert $tmpA -define convolve:scale=\! -convolve "$ave" $tmpM

# get std = sqrt( ave(x^2) - ave(x)^2 )
# -gamma 2 is equivalent to sqrt
convert \( $tmpA $tmpA -compose multiply -composite -define convolve:scale=\! -convolve "$ave" \) \
