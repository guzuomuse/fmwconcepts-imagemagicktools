#!/bin/bash
#
# Developed by Fred Weinhaus 3/31/2010 .......... revised 3/31/2010
#
...
# NAME: INNERCROP 
# 
# PURPOSE: To crop an image to the largest rectangle that will just trim 
# any irregularly shaped outer boundary color.
# 
# DESCRIPTION: INNERCROP crops an image to the largest rectangle that will 
			...
			case "$1" in
		     -help)    # help information
				...
				-i)    # get is360test
					   ...
					   [ "$is360test" != "yes" -a "$is360" != "no" ] && errMsg "--- IS360=$is360test MUST BE EITHER YES OR NO ---"
					   ;;
				-r)    # get rmse
					   shift  # to get the next parameter
					   # test if parameter starts with minus sign 
...
if [ $prep -eq 1 ]; then
	convert -quiet -regard-warnings "$infile" -fuzz ${fuzzval}% -trim "$tmpA1" || 
		errMsg  "--- FILE $imagefile DOES NOT EXIST OR IS NOT AN ORDINARY FILE, NOT READABLE OR HAS ZERO SIZE  ---"
elif [ $prep -eq 2 ]; then
echo "fuzzval=${fuzzval}%"
	convert -quiet -regard-warnings "$infile" -bordercolor "$ocolor" -border 2 \
...
procVertHoriz()
	...
	thresh2=`convert xc: -ping -format "%[fx:100-$thresh1]" info:`
	
	# get thresholded row or column from potentially cropped mask
	# also compute length for array
	if [ "$direction" = "vertical" ]; then
		convert $tmpA2[${width}x${height}+$xoff+$yoff] -scale 1x${height}! -threshold ${thresh2}% $tmpA3
		len=$height
		len1=`convert xc: -format "%[fx:$height-1]" info:`
	elif [ "$direction" = "horizontal" ]; then
		convert $tmpA2[${width}x${height}+$xoff+$yoff] -scale ${width}x1! -threshold ${thresh2}% $tmpA3
		len=$width
