#!/bin/bash
#
# Developed by Fred Weinhaus 5/29/2009 .......... revised 6/9/2009
#
# USAGE: whiteboard  [-c coords] [-a aspect ] [-m magnify] [-d dimensions] [-e enhance ] [-f filtersize] [-o offset] [-t threshold]  [-s sharpen] [-S saturation] [-w whitecolor] infile outfile
...
#                           1.33 (4:3); floats>0; The default=1
...
aspect=1			# typical width-to-height ratios of 2, 1.5 or 1.33
...
whiteBalance()
	...
	convert $tmpA1 -colorspace HSB -channel G -negate -channel GB -separate \
...
#echo "coords=$coords"
	...
	if [ "$dimensions" != "" -a "$ww" = "" -a "$hh" != "" ]; then
		...
		width=`convert $tmpA1 -ping -format "%[fx:$magnify*w]" info:`
		height=`convert $tmpA1 -ping -format "%[fx:$magnify*h]" info:`
	fi

	# compute offsets for virtual canvas
	delx=`convert $tmpA1 -ping -format "%[fx:(w-$width)/2]" info:`
	dely=`convert $tmpA1 -ping -format "%[fx:(h-$height)/2]" info:`
...
if [ "$enhance" = "stretch" ]; then
	convert $tmpA1 -contrast-stretch 0 $tmpA1
...
if [ "$coords" = "" -a "$magnify" = "1" -a "$dimensions" = "" ]; then
	convert \( $tmpA1 \) \
		\( -clone 0 -colorspace gray -negate -lat ${filtersize}x${filtersize}+${offset}% -contrast-stretch 0 $blurring \) \
